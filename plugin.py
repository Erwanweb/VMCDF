...
def compute_room_td_list(self):
    ...
    for i, idx in enumerate(self.hum_idxs):
        ...
        if RH is None:
            if self.debug:
                Domoticz.Debug(f"ΔTd: idx={idx} RH=NA (pas de valeur) – ignoré")
            continue
        ...
        try:
            Td_ext = self.last_values.get("Td_ext")
            Domoticz.Debug(
                f"ΔTd: idx={idx} RH(raw+off)={fmt1(RH,'%')} (off={off:+.1f}) "
                f"T_room={fmt1(T_room,'°C')} Td={fmt1(Td,'°C')} vs Td_ext={fmt1(Td_ext,'°C')}"
            )
        except Exception as e:
            Domoticz.Debug(f"ΔTd: idx={idx} [WARN log] {e}")
    ...
    if per_sensor_logs:
        Domoticz.Debug("ΔTd recap: " + " | ".join(per_sensor_logs))
