...
class BasePlugin:
    ...
    def refresh_and_act(self):
        ...
        if avg_hum is not None:
            ...
            # Pour un device Humidity, Domoticz attend sValue="HUM;STATUS"
            Devices[1].Update(nValue=0, sValue=f"{hum_int};{status}")
        else:
            # Valeur inconnue -> status Normal(0)
            Devices[1].Update(nValue=0, sValue="0;0")
    ...
    def get_hum_status(self, hum_int):
        """Mappe l'humidité vers le statut Domoticz.
        0=Normal (fallback), 1=Comfortable, 2=Dry, 3=Wet
        Ici : Dry si <= seuil bas, Wet si >= seuil haut, sinon Comfortable.
        """
        try:
            if hum_int is None:
                return 0
            if hum_int <= int(round(self.low_th)):
                return 2  # Dry
            if hum_int >= int(round(self.high_th)):
                return 3  # Wet
