...
class BasePlugin:
        ...
        if self.force_mode:
            ...
            if Td_ext is None or Td_target is None:
                ...
                if gate == 'SEC':
                    ...
                    RH_int = self.last_values.get("RH_int")
                    RH_ext = self.last_values.get("RH_ext")
                    Td_ext = self.last_values.get("Td_ext")

                    # Plancher physique à T_int selon Td_ext (au cas où)
                    if (T_int is not None) and (Td_ext is not None):
                        rh_floor = rh_from_t_and_tdew(T_int, Td_ext)  # RH mini atteignable à T_int
                    else:
                        rh_floor = None

                    # Coupe basée sur la moyenne (RH_int + RH_ext)/2 + offset
                    # (offset interne, sans UI)
                    if not hasattr(self, "_hum_cut_offset"):
                        self._hum_cut_offset = 1.5  # entre 1 et 2 % conseillé
                    if (RH_int is not None) and (RH_ext is not None):
                        avg_cut = (RH_int + RH_ext) / 2.0 + self._hum_cut_offset
                        avg_cut = min(100.0, max(0.0, avg_cut))
                    else:
                        avg_cut = None

                    # Seuil d'arrêt effectif
                    low_eff = self.low_th
                    if rh_floor is not None:
                        low_eff = max(low_eff, rh_floor)
                    if avg_cut is not None:
                        low_eff = max(low_eff, avg_cut)
                    low_eff = min(100.0, low_eff)

                    # Règles ON/OFF en HUMIDE
                    ...
                    Domoticz.Debug(
                        f"HUMIDE: low_th={self.low_th:.1f} rh_floor={rh_floor:.1f if rh_floor is not None else '-'} "
                        f"avg_cut={avg_cut:.1f if avg_cut is not None else '-'} -> low_eff={low_eff:.1f}"
                    )
    ...
    def post_state(self, mode_label, target_on):
        ...
        if Td_ext is not None and Td_target is not None:
            try:
                Domoticz.Debug(
                    f"-----DEBUG : T_ext={self.last_values.get('T_ext'):.1f}°C / RH_ext={self.last_values.get('RH_ext'):.1f}% | "
