...
class BasePlugin:
    ...
    def refresh_and_act(self):
        ...
        T_wet, RH_wet = avg_T_RH_from_idxs(self.hum_idxs, self.get_device_by_idx)

        if avg_hum is not None and T_wet is not None:
            T_val = round(T_wet, 1)
            H_val = int(round(avg_hum))
            status = self.get_hum_status(H_val)
            svalue = f"{T_val:.1f};{H_val};{status}"
            Devices[1].Update(nValue=0, sValue=svalue)

        # --- Update Device 4: Temp+Hum moyennes des pièces normales ---
        if (T_int is not None) and (RH_int is not None):
            t_val = round(float(T_int), 1)
            h_val = int(round(float(RH_int)))
            status = self.get_hum_status(h_val)  # 0..3 selon ta fonction
            # Format Domoticz pour Temp+Hum : "Temp;Humidity;Status"
