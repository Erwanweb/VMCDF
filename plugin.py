...
<plugin key="ZZ-VMCDF" name="RONELABS - VMC DF Control" author="ErwanBCN" version="1.0.1" externallink="https://ronelabs.com">
    ...
    <params>
        ...
        <param field="Mode5" label="Params(expert)- Timer(Min),L-HR,H-HR,ΔTd-DRY,ΔTd-ON,ΔTd-OFF " width="400px" required="true" default="60,55,75,20,10,5"/>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if len(params) == 6: # Timer(Min),L/H-HR,ΔTd-DRY,ΔTd-ON,ΔTd-OFF
            ...
            self.low_th = CheckParam("L-HR", params[1], 55) # LOW Humidity threshold - DRY Mode
            self.high_th = CheckParam("H-HR", params[2], 75) # HIGH Humidity threshold - DRY Mode
            self._td_eps = CheckParam("ΔTd-DRY,", params[3], 20) # marge 'nettement plus sec' pour gate SEC (°C)
        ...
        created = []
        if 1 not in Devices:
            Domoticz.Device(Unit=1, Name="Avg Wet Rooms Hum", TypeName="Humidity", Used=1).Create()
            created.append(1)
        if 2 not in Devices:
        ...
        # Set Auto mode if first starting
    ...
    def onCommand(self, Unit, Command, Level, Color):
        ...
        if Unit == 3:  # Mode
            Devices[1].Update(nValue=1, sValue=str(Level))
            if (Devices[3].sValue == "10"):  # Mode Auto
                self.force_mode = False
                Devices[3].Update(nValue=1, sValue="10")
            elif (Devices[3].sValue == "20"):  # Mode Timer
                self.force_mode = True
                self.TimerStartedTime = now
                self.TimerOn = True
                Devices[3].Update(nValue=1, sValue="20")
            elif (Devices[3].sValue == "30"):  # Forced Mode
                self.force_mode = True
                Devices[3].Update(nValue=1, sValue="30")
            else : # Off hidden so Auto mode
                self.force_mode = False
                Devices[3].Update(nValue=1, sValue="10")
        ...
        if self.force_mode:
            ...
            if gate == 'SEC':
                ...
                try:
                    ...
                    Domoticz.Debug(
                        f"--------------DEBUG : WET MODE | On@≥ {td_on:+.1f} & Off@≤ {td_off:+.1f} | ΔTd: Td_ext(°C)= {Td_ext:.1f} | Td_rooms(°C)= [{lst}] | Boost -> {'ON' if target_on else 'OFF' if target_on is False else 'HOLD'}")
                except Exception:
                    pass
    ...
    def post_state(self, mode_label, target_on):
        if target_on is None:
            ...
            txt = f"{mode_label} — Boost is {'ON' if target_on else 'OFF'}"
        Devices[2].Update(nValue=0, sValue=txt)
        Domoticz.Debug(f"--------------DEBUG : {mode_label} | Boost -> {'ON' if target_on else 'OFF' if target_on is False else 'HOLD'}")
