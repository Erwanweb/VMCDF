...
class BasePlugin:
    ...
    def switch_relay(self, on):
        ...
        if cur_state == desired:
            if self.debug:
                Domoticz.Debug(f"Relay idx {self.relay_idx}: already {desired}, skipping command.")
            return True
        ...
        if self.debug:
            Domoticz.Debug(f"Relay idx {self.relay_idx}: sent {cmd} (prev={cur_state or 'unknown'})")
        return True
