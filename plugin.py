...
class BasePlugin:
    def __init__(self):
        ...
        self._ext_offset = 2.0  # offset d’arrêt en HUMIDE : RH_ext + 2 % (mets 1.0 si tu préfères)
    ...
    def apply_control(self):
        ...
        if self.force_mode:
            ...
            if Td_ext is None or Td_target is None:
                ...
                # --- Gate SEC/HUMIDE (logique simplifiée) ---
                eps = 2.0  # marge "vraiment plus sec"
                ...
                if gate == 'SEC':
                    ...
                    # Seuil d'arrêt effectif : RH_ext + offset (borné 0–100). Si RH_ext inconnu -> repli sur low_th
                    if RH_ext is not None:
                        low_eff = min(100.0, max(0.0, RH_ext + self._ext_offset))
                    else:
                        low_eff = self.low_th
                    ...
                    # Debug propre (évite les .1f sur None)
                    rh_ext_str = "-" if RH_ext is None else f"{RH_ext:.1f}"
                    hum_list_str = ", ".join(f"{v:.1f}" for v in hum_vals) if hum_vals else "-"
                    Domoticz.Debug(
                        f"-----DEBUG : HUMIDE: RH_ext={rh_ext_str}% + offset={self._ext_offset:.1f} -> low_eff={low_eff:.1f} | hum_list=[{hum_list_str}]"
                    )
