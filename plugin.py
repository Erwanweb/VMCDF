...
<plugin key="ZZ-VMCDF" name="RONELABS - VMC DF Control" author="ErwanBCN" version="1.0.1" externallink="https://ronelabs.com">
    ...
    <params>
        ...
        <param field="Mode5" label="Params(expert)- Timer(Min),L/H-HR,ΔTd-DRY,ΔTd-ON,ΔTd-OFF " width="400px" required="true" default="60,55,75,20,10.5"/>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if len(params) == 6: # Timer(Min),L/H-HR,ΔTd-DRY,ΔTd-ON,ΔTd-OFF
            ...
            self._td_off = heckParam("ΔTd-OFF", params[5], 5) # ΔTd pour arrêt BOOST HUMIDE (°C) - WET Mode
...
def compute_room_td_list(self):
    ...
    for i, idx in enumerate(self.hum_idxs):
        ...
        # ---- DEBUG PAR SONDE (sécurisé) ----
        try:
            Td_ext = self.last_values.get("Td_ext")
            Domoticz.Debug(
                f"--------------DEBUG :ΔTd: idx={idx} RH(raw+off)={fmt1(RH,'%')} (off={off:+.1f}) "
                f"T_room={fmt1(T_room,'°C')} Td={fmt1(Td,'°C')} vs Td_ext={fmt1(Td_ext,'°C')}"
            )
        except Exception as e:
            Domoticz.Debug(f"--------------DEBUG : ΔTd: idx={idx} [WARN log] {e}")

    # Construit aussi un récap
    per_sensor_logs.append(f"{idx}:{RH:.1f}%/{T_room:.1f}°C→Td={Td:.1f}°C")

    # ---- RÉCAP FINAL (optionnel) ----
    if per_sensor_logs:
        Domoticz.Debug(f"--------------DEBUG : ΔTd recap: " + " | ".join(per_sensor_logs))
